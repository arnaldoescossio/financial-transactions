...
async def test_get_transactions_by_account_success_with_default_status(
    mock_service, mock_transactions
):
    """Test successful retrieval of transactions with default SUCCESS status."""
    # Arrange
    account_id = 1
    mock_service.get_transactions_by_account_id = AsyncMock(return_value=mock_transactions)
    use_case = GetTransactionsByAccountUseCase(mock_service)

    # Act
    result = await use_case.execute(dict(account_id=account_id))
    ...
    mock_service.get_transactions_by_account_id.assert_called_once_with(
...
async def test_get_transactions_by_account_with_pending_status(
    ...
    mock_service.get_transactions_by_account_id = AsyncMock(return_value=[mock_transaction])
    use_case = GetTransactionsByAccountUseCase(mock_service)

    # Act
    result = await use_case.execute(
        dict(account_id=account_id, transaction_status=TransactionStatus.PENDING)
    )
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_with_failed_status(mock_service):
    ...
    # Act
    result = await use_case.execute(
        dict(account_id=account_id, transaction_status=TransactionStatus.FAILED)
    )
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_empty_result(mock_service):
    """Test retrieval when no transactions exist for the account."""
    # Arrange
    account_id = 999
    mock_service.get_transactions_by_account_id = AsyncMock(return_value=[])
    use_case = GetTransactionsByAccountUseCase(mock_service)
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_single_transaction(mock_service):
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_many_transactions(mock_service):
...
async def test_get_transactions_by_account_response_structure(
    ...
    # Arrange
    account_id = 1
    mock_service.get_transactions_by_account_id = AsyncMock(return_value=[mock_transactions[0]])
    use_case = GetTransactionsByAccountUseCase(mock_service)
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_preserves_all_fields(mock_service):
...
async def test_get_transactions_by_account_repository_called_with_correct_params(
    ...
    """Test that repository.get_transactions_by_account_id is called with correct parameters."""
    # Arrange
    account_id = 7
    ...
    await use_case.execute(
        dict(account_id=account_id, transaction_status=transaction_status)
    )
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_with_zero_amount(mock_service):
...
@pytest.mark.asyncio
async def test_get_transactions_by_account_with_large_amount(mock_service):
...
async def test_get_transactions_by_account_mixed_statuses_with_filter(
    ...
    mock_service.get_transactions_by_account_id = AsyncMock(return_value=success_transactions)
    use_case = GetTransactionsByAccountUseCase(mock_service)

    # Act
    result = await use_case.execute(
        dict(account_id=account_id, transaction_status=TransactionStatus.SUCCESS)
    )
