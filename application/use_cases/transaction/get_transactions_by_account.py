...
class GetTransactionsByAccountUseCase(UseCase):
    ...
    @override
    async def execute(self, account_id: int, transaction_status: TransactionStatus = TransactionStatus.SUCCESS) -> list[TransactionBase]:
        transactions = await self.repository.get_by_account_id(account_id, transaction_status)
